...
etcd_iptables:
  file.accumulated:
    ...
    - text: |
        ...
        -A JPL-INPUT-LOCAL-ACCEPT -s 128.149.16.0/24 -p tcp -m state --state NEW -m tcp --dport 4001 -j ACCEPT
        -A JPL-INPUT-LOCAL-ACCEPT -s 128.149.16.0/24 -p tcp -m state --state NEW -m tcp --dport 7001 -j ACCEPT
    - require_in:
        - file: salt_iptables
