...
postgresql_db_iptables:
  file.accumulated:
    ...
    - text: |
        ...
        -A JPL-INPUT-LOCAL-ACCEPT -s 128.149.18.0/24,128.149.19.0/24,128.149.124.0/24 -p tcp -m state --state NEW -m tcp --dport 5432 -j ACCEPT
        -A JPL-INPUT-LOCAL-ACCEPT -s 10.0.0.0/24 -p tcp -m state --state NEW -m tcp --dport 5432 -j ACCEPT
    - require_in:
        - file: salt_iptables
